...
router.post("/", validateToken, async (req, res) => {
  ...
  const results = await response.results.map((page) => {
    return {
      id: page.id,
      personName:
        page.properties.Person_Name.rollup.array[0].title[0]
          .plain_text,
      projectName:
        page.properties.Project_Name.rollup.array[0]
          .title[0].plain_text,
  ...
  async (req, res) => {
    ...
    const results = await response.results.map((page) => {
      return {
        id: page.id,
        personName:
          page.properties.Person_Name.rollup.array[0]
            .title[0].plain_text,
        projectName:
          page.properties.Project_Name.rollup.array[0]
            .title[0].plain_text,
...
router.post("/unique", validateToken, async (req, res) => {
    const id = req.user.id;
    const person = req.body.person;
    const project = req.body.project;
    const dateFrom = req.body.dateFrom;
